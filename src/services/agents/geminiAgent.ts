...
export class GeminiAgent {
  ...
  static async generateMorningBriefing(marketData: any): Promise<string> {
    if (!ai || isGeminiSuspended()) {
      return `🌅 PRISMX BRIEF (FALLBACK ACTIVE)
-----------------------------------------------
...
⚠️ Risk: US FOMC minute release tonight. Safe-haven volatility swings may affect tomorrow's open.`;
    ...
    const prompt = `
      You are PRISMX AI, an elite AI portfolio prediction and strategy assistant for Indian retail gold/silver investors.
      ...
      🌅 PRISMX BRIEF — ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
...
💡 Suggestion: Portfolio sizing control me rakhe aur systematic systematic SIP limits scale up karein.`;
    ...
    const prompt = `
      You are PRISMX AI, the friendly AI portfolio manager.
      Explain in simple conversational Hindi-English (Hinglish) why PRISMX AI has compiled a **${signal}** recommendation for asset ${symbol}.
  ...
  static async generateWeeklyReport(performanceData: any): Promise<string> {
    ...
    return `📊 PRISMX WEEKLY PORTFOLIO AUDIT
-----------------------------------------------

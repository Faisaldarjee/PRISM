...
export function SmartSwing() {
  ...
  const handleShareSetup = (symbol: string) => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(`PRISMX Swing Trade Plan for ${symbol}: Target ₹${deepDiveStock?.trade_plan?.target_1 || deepDiveStock?.target1}, Stop Loss ₹${deepDiveStock?.trade_plan?.stop_loss || deepDiveStock?.stopLoss}`);
    }
    setTimeout(() => {
